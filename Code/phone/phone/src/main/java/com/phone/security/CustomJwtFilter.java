...
public class CustomJwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        String token = getTokenFromHedear(request);
        if (token != null){
    ...
    private String getTokenFromHedear(HttpServletRequest request){
        String hedear = request.getHeader("Authorization");
        String token = null;
        if (StringUtils.hasText(hedear) && hedear.startsWith("Bearer")){
            token = hedear.substring(7);
        }
        return token;
    }
}
